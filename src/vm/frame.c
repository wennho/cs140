...
void
frame_deallocate (void *vaddr, bool is_in_swap, block_sector_t sector)
{
          ...
          if (evict_data->is_mapped)
            {
              ...
              if (!evict_data->backing_file->is_segment)
                {
                  backed_file_write_back_page (evict_data->backing_file,
                      evict_data->file_offset, evict_data->readable_bytes);
                }
...
struct frame *
frame_get_new (void *vaddr, bool user, struct page_data* data, bool pin_data)
{
  ...
  if (data != NULL)
    {
      ...
      if (!pagedir_set_page (data->pagedir, vaddr, paddr, data->is_writable))
        {
          frame_deallocate_paddr (paddr);
          exit (-1);
        }
...
struct frame *
frame_get_from_swap (struct page_data * data, bool user)
{
...
void
frame_load_data (struct page_data* data, bool user)
{
